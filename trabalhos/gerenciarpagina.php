...
?>
   ...
   <table align="center">
    ...
    <?php
        echo('<tr><td><input type="hidden" name="valor" value="'.$valor.'"></td></tr>');
        echo('<tr><td><input type="hidden" name="titulo" value="'.$titulo.'"></td></tr>');
        echo('<tr><td><input type="hidden" name="imagem" value="'.$imagem.'"></td></tr>');
        echo('<tr><td><input type="hidden" name="descriver" value="'.$descricao.'"></td></tr>');
        echo('<tr><td><input type="hidden" name="categoria" value="'.$categoria.'"></td></tr>');
        echo('<tr><td><input type="submit" name="alterar" value="Alterar"></td></tr>');
    ?>
  </form>
      <form method="POST" action="gerenciarpagina.php" enctype="multipart/form-data" onsubmit="return confirmacao();">
    <?php
    $consulta = 'select codigo from produtos where titulo="'.$titulo.'" and descricao="'.$descricao.'" and valor="'.$valor.'" and categorias="'.$categoria.'"  and imagem="'.$imagem.'"';
    $resultado = mysql_query($consulta,$link);
    echo(mysql_error());
    while($linha  =  mysql_fetch_array($resultado)){
    $produto=$linha['codigo'];
    }
        echo('<tr><td><input type="hidden" name="produto" value="'.$produto.'"></td></tr>');
        echo('<tr><td><input type="submit" name="deletar" value="Deletar"></td></tr>');
    ?>
   </table>
  </form>
        </div>
...
<?php
   if (isset($_POST['deletar'])){
    $consulta = 'delete from produtos where codigo="'.$_POST['produto'].'"';
    $resultado = mysql_query($consulta,$link);
    echo(mysql_error());
